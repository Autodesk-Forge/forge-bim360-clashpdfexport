...
router.get('/mc/clash/getBreakdownData/:mc_container_id/:ms_id/:ms_v_id', async (req, res, next) => {

  try {  
    const mc_container_id = req.params['mc_container_id']  
    const ms_id = req.params['ms_id']
    const ms_v_id = req.params['ms_v_id']  
    const breakdownData = analyze.getBreakdownData(mc_container_id,ms_id,ms_v_id) 
    if(!breakdownData)
        res.status(500).end('breakdown data is null')
    else
...
router.get('/mc/clash/getMatrixData/:mc_container_id/:ms_id/:ms_v_id', async (req, res, next) => {
  ...
  try {
    ...
    const ignoredAssinedClash = false
    const matrixView = await analyze.getMatrixData(mc_container_id,ms_id, ms_v_id,ignoredAssinedClash) 
    if(!matrixView)
       res.json({error:'matrixView null'})

    res.json(matrixView) 
 
   } catch(e) {
      // here goes out error handler
      res.json({error:'get matrixView failed!'})
  }   
}); 
